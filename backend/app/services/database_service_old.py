...
class DatabaseService:
        ...
        try:
            with next(get_session()) as session:
                state = session.exec(
                    ...
                    .where(AgentState.user_id == self.user_id)
                    .where(AgentState.agent_name == agent_name)
                    .order_by(AgentState.timestamp.desc())
                ).first()
 ...
 signatures = session.exec(
                    ...
                    .order_by(ThoughtSignature.timestamp.desc())
                    .limit(limit)
                ).all()
                ...
                for job_data in listings:
                    ...
                    if not existing:
                        job_listing = JobListing(
                            ...
                            source=source,
                            skills_extracted=job_data.get("skills", [])
                        )
                        session.add(job_listing)
                
                session.commit()
                logging.info(f"[DB] Saved {len(listings)} job listings from {source}")
